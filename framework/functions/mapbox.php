	...
	function realhomes_enqueue_mapbox() {
		...
		if ( inspiry_is_map_needed() || inspiry_map_needed_for_address() ) {
			...
			if ( is_singular( 'property' ) && ( 'true' == get_option( 'theme_display_google_map' ) ) ) {
				...
				realhomes_current_template( 'page', array( 'contact' ) )
				&& ( '1' == get_post_meta( get_the_ID(), 'theme_show_contact_map', true ) )
			) {

				realhomes_render_contact_mapbox();

			} elseif ( realhomes_current_template( 'page', array( 'submit-property', 'dashboard' ) ) ) {

				realhomes_render_submit_mapbox();

			} elseif ( realhomes_current_template( 'page', array( 'half-map-layout' ) ) ) {

				realhomes_render_multi_properties_mapbox();

			} elseif ( realhomes_current_template( 'page', array( 'home' ) ) ) {
...
if ( ! function_exists( 'realhomes_render_contact_mapbox' ) ) {
	...
	function realhomes_render_contact_mapbox() {
		...
		wp_register_script(
			'contact-mapbox',
			get_theme_file_uri( 'assets/' . INSPIRY_DESIGN_VARIATION . '/scripts/js/contact-mapbox.js' ),
			array( 'jquery' ),
			INSPIRY_THEME_VERSION,
			true
		);
		...
		$contact_marker_color     = get_post_meta( get_the_ID(), 'inspiry_contact_marker_color', true );
...
if ( ! function_exists( 'realhomes_render_multi_properties_mapbox' ) ) {
	...
	function realhomes_render_multi_properties_mapbox() {
		...
		wp_register_script(
			'properties-mapbox',
			get_theme_file_uri( 'assets/' . INSPIRY_DESIGN_VARIATION . '/scripts/js/properties-mapbox.js' ),
...
if ( ! function_exists( 'realhomes_render_property_mapbox' ) ) {
	...
	function realhomes_render_property_mapbox() {
		...
		wp_register_script(
			'property-mapbox',
			get_theme_file_uri( 'assets/' . INSPIRY_DESIGN_VARIATION . '/scripts/js/property-mapbox.js' ),
		...
		if ( $property_address && ! empty( $property_location ) && ( 1 != $property_map ) ) {
			...
			$property_map_data['propertyType'] = ere_get_property_types( get_the_ID() );
...
if ( ! function_exists( 'realhomes_render_properties_on_mapbox' ) ) {
	...
	function realhomes_render_properties_on_mapbox() {
		...
		} elseif ( in_array( $current_page_template, array( 'templates/list-layout.php', 'templates/list-layout-full-width.php', 'templates/grid-layout.php', 'templates/grid-layout-full-width.php', 'templates/half-map-layout.php' ) ) ) {
		...
		} elseif ( ! empty( $is_taxonomy ) && 'true' === $is_taxonomy ) {
