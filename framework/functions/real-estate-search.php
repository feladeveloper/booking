...
if ( ! function_exists( 'inspiry_show_search_form_widget' ) ) {
	...
	function inspiry_show_search_form_widget() {
		...
		$current_page_search_form      = get_post_meta( get_queried_object_id(), 'REAL_HOMES_hide_advance_search', true );
...
if ( ! function_exists( 'inspiry_hide_empty_locations' ) ) :
	...
	function inspiry_hide_empty_locations(): bool {
		if ( ( 'true' == get_option( 'inspiry_hide_empty_locations', 'true' ) )
		     && ( ! is_page_template( array( 'templates/dashboard.php' ) ) ) ) {
			return true;
		}
...
if ( ! function_exists( 'realhomes_get_edited_property_locations' ) ) {
	...
	function realhomes_get_edited_property_locations( int $location_select_count, $location_select_names ): array {
		...
		if ( is_page_template( 'templates/dashboard.php' ) && isset( $_GET['id'] ) ) {
...
if ( ! function_exists( 'realhomes_agency_options_for_search' ) ) {
	/**
	 * Generate Agencies options for Advanced Search Form
	 * 
	 * @since 3.21.0
	 */
	function realhomes_agency_options_for_search() {
		$agencies = ERE_Data::get_agencies_id_name();
		...
		$multiselect_for_agency_field = get_option( 'realhomes_search_form_multiselect_agencies', 'yes' );

		if ( 'no' == $multiselect_for_agency_field ) {
			$agent_placeholder = get_option( 'realhomes_property_agency_placeholder' );
		...
		if ( ! empty( $agencies ) ) {
			foreach ( $agencies as $agency_id => $agency_name ) {
				if ( is_array( $searched_value ) && in_array( $agency_id, $searched_value ) ) {
					echo '<option value="' . intval( $agency_id ) . '" selected="selected">' . esc_html( $agency_name ) . '</option>';
				} else {
					echo '<option value="' . intval( $agency_id ) . '">' . esc_html( $agency_name ) . '</option>';
				}
			}
		}
	}
...
if ( ! function_exists( 'inspiry_agents_in_search' ) ) {
	/**
	 * Generate Agents options for Advanced Search Form
	 */
...
if ( ! function_exists( 'realhomes_lot_size_search' ) ) :
	/**
	 * Add lot size related search arguments to meta query
	 *
	 * @param $meta_query
	 *
	 * @return array
	 */
	function realhomes_lot_size_search( $meta_query ) {

		if ( isset( $_GET['min-lot-size'] ) && ! empty( $_GET['min-lot-size'] ) && isset( $_GET['max-lot-size'] ) && ! empty( $_GET['max-lot-size'] ) ) {
			$min_lot_size = intval( $_GET['min-lot-size'] );
			$max_lot_size = intval( $_GET['max-lot-size'] );
		...
		} elseif ( isset( $_GET['min-lot-size'] ) && ! empty( $_GET['min-lot-size'] ) ) {
			$min_lot_size = intval( $_GET['min-lot-size'] );
			if ( $min_lot_size > 0 ) {
				$meta_query[] = array(
					'key'     => 'REAL_HOMES_property_lot_size',
		...
		} elseif ( isset( $_GET['max-lot-size'] ) && ! empty( $_GET['max-lot-size'] ) ) {
			$max_lot_size = intval( $_GET['max-lot-size'] );
			if ( $max_lot_size > 0 ) {
				$meta_query[] = array(
					'key'     => 'REAL_HOMES_property_lot_size',
	...
	add_filter( 'inspiry_real_estate_meta_search', 'realhomes_lot_size_search' );
endif;
...
if ( ! function_exists( 'realhomes_agency_search' ) ) :
	/**
	 ...
	 * @since 3.21.0
	 */
	function realhomes_agency_search( $meta_query ) {
		if ( isset( $_GET['agencies'] ) && ! empty( $_GET['agencies'] ) ) {
	...
	add_filter( 'inspiry_real_estate_meta_search', 'realhomes_agency_search' );
endif;
