...
if ( ! function_exists( 'inspiry_property_submit_redirect' ) ) {
	 ...
	 */
	function inspiry_property_submit_redirect( $updated = false, $return = false, $property_id = NULL ) {
...
if ( ! function_exists( 'inspiry_render_floor_plans' ) ) {
	...
	function inspiry_render_floor_plans( $index = '0', $floor_plan = array() ) {
		...
		$floor_name_label = get_option( 'realhomes_submit_property_floor_name_label' );
		if ( empty( $floor_name_label ) ) {
			$floor_name_label = esc_html__( 'Floor Name', 'framework' );
		}

		$floor_plan_image_label = get_option( 'realhomes_submit_property_floor_image_label' );
		if ( empty( $floor_plan_image_label ) ) {
			$floor_plan_image_label = esc_html__( 'Floor Plan Image', 'framework' );
		}

		$floor_plan_description_label = get_option( 'realhomes_submit_property_floor_description_label' );
		if ( empty( $floor_plan_description_label ) ) {
			$floor_plan_description_label = esc_html__( 'Description', 'framework' );
		}

		$floor_price_label = get_option( 'realhomes_submit_property_floor_price_label' );
		if ( empty( $floor_price_label ) ) {
			$floor_price_label = esc_html__( 'Floor Price', 'framework' );
		}

		$floor_price_postfix_label = get_option( 'realhomes_submit_property_floor_price_postfix_label' );
		if ( empty( $floor_price_postfix_label ) ) {
			$floor_price_postfix_label = esc_html__( 'Price Postfix', 'framework' );
		}

		$floor_size_label = get_option( 'realhomes_submit_property_floor_size_label' );
		if ( empty( $floor_size_label ) ) {
			$floor_size_label = esc_html__( 'Floor Size', 'framework' );
		}

		$floor_size_postfix_label = get_option( 'realhomes_submit_property_floor_size_postfix_label' );
		if ( empty( $floor_size_postfix_label ) ) {
			$floor_size_postfix_label = esc_html__( 'Size Postfix', 'framework' );
		}

		$floor_bedrooms_label = get_option( 'realhomes_submit_property_floor_beds_label' );
		if ( empty( $floor_bedrooms_label ) ) {
			$floor_bedrooms_label = esc_html__( 'Bedrooms', 'framework' );
		}

		$floor_bathrooms_label = get_option( 'realhomes_submit_property_floor_baths_label' );
		if ( empty( $floor_bathrooms_label ) ) {
			$floor_bathrooms_label = esc_html__( 'Bathrooms', 'framework' );
		}

		$floor_plan_name          = isset( $floor_plan['inspiry_floor_plan_name'] ) ? $floor_plan['inspiry_floor_plan_name'] : '';
		$floor_plan_descr         = isset( $floor_plan['inspiry_floor_plan_descr'] ) ? $floor_plan['inspiry_floor_plan_descr'] : '';
		$floor_plan_price         = isset( $floor_plan['inspiry_floor_plan_price'] ) ? $floor_plan['inspiry_floor_plan_price'] : '';
		$floor_plan_price_postfix = isset( $floor_plan['inspiry_floor_plan_price_postfix'] ) ? $floor_plan['inspiry_floor_plan_price_postfix'] : '';
		$floor_plan_size          = isset( $floor_plan['inspiry_floor_plan_size'] ) ? $floor_plan['inspiry_floor_plan_size'] : '';
		$floor_plan_size_postfix  = isset( $floor_plan['inspiry_floor_plan_size_postfix'] ) ? $floor_plan['inspiry_floor_plan_size_postfix'] : '';
		$floor_plan_bedrooms      = isset( $floor_plan['inspiry_floor_plan_bedrooms'] ) ? $floor_plan['inspiry_floor_plan_bedrooms'] : '';
		$floor_plan_bathrooms     = isset( $floor_plan['inspiry_floor_plan_bathrooms'] ) ? $floor_plan['inspiry_floor_plan_bathrooms'] : '';
		$floor_plan_image         = isset( $floor_plan['inspiry_floor_plan_image'] ) ? $floor_plan['inspiry_floor_plan_image'] : '';
		?>
        <div class="inspiry-clone inspiry-group-clone" data-floor-plan="<?php echo esc_attr( $index ); ?>">
            <div class="row">
                <div class="col-lg-6">
                    <div class="inspiry-field">
                        <label for="inspiry_floor_plan_name_<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $floor_name_label ); ?></label>
                        <input type="text" id="inspiry_floor_plan_name_<?php echo esc_attr( $index ); ?>" name="inspiry_floor_plans[<?php echo esc_attr( $index ); ?>][inspiry_floor_plan_name]" value="<?php echo esc_attr( $floor_plan_name ); ?>">
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="inspiry-field inspiry-file-input-wrapper">
                        <label><?php echo esc_html( $floor_plan_image_label ); ?><span><?php esc_html_e( '* Minimum width is 770px and height is flexible.', 'framework' ); ?></span></label>
						<?php
                        ...
                        <div class="inspiry-btn-group clearfix <?php echo esc_attr( $inspiry_show_remove_btn ); ?>">
                            <input type="text" class="inspiry-file-input" name="inspiry_floor_plans[<?php echo esc_attr( $index ); ?>][inspiry_floor_plan_image]" value="<?php echo esc_attr( $floor_plan_image ); ?>">
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="inspiry-field">
                        <label for="inspiry_floor_plan_descr_<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $floor_plan_description_label ); ?></label>
                        <textarea id="inspiry_floor_plan_descr_<?php echo esc_attr( $index ); ?>" class="inspiry-textarea" name="inspiry_floor_plans[<?php echo esc_attr( $index ); ?>][inspiry_floor_plan_descr]"><?php echo esc_attr( $floor_plan_descr ); ?></textarea>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="inspiry-field">
                                <label for="inspiry_floor_plan_price_<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $floor_price_label ); ?> <span><?php esc_html_e( '( Only digits )', 'framework' ); ?></span></label>
                                <input type="text" id="inspiry_floor_plan_price_<?php echo esc_attr( $index ); ?>" name="inspiry_floor_plans[<?php echo esc_attr( $index ); ?>][inspiry_floor_plan_price]" value="<?php echo esc_attr( $floor_plan_price ); ?>">
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="inspiry-field">
                                <label for="inspiry_floor_plan_price_postfix_<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $floor_price_postfix_label ); ?></label>
                                <input type="text" id="inspiry_floor_plan_price_postfix_<?php echo esc_attr( $index ); ?>" name="inspiry_floor_plans[<?php echo esc_attr( $index ); ?>][inspiry_floor_plan_price_postfix]" value="<?php echo esc_attr( $floor_plan_price_postfix ); ?>">
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="inspiry-field">
                                <label for="inspiry_floor_plan_size_<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $floor_size_label ); ?> <span><?php esc_html_e( '( Only digits )', 'framework' ); ?></span></label>
                                <input type="text" id="inspiry_floor_plan_size_<?php echo esc_attr( $index ); ?>" name="inspiry_floor_plans[<?php echo esc_attr( $index ); ?>][inspiry_floor_plan_size]" value="<?php echo esc_attr( $floor_plan_size ); ?>">
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="inspiry-field">
                                <label for="inspiry_floor_plan_size_postfix_<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $floor_size_postfix_label ); ?></label>
                                <input type="text" id="inspiry_floor_plan_size_postfix_<?php echo esc_attr( $index ); ?>" name="inspiry_floor_plans[<?php echo esc_attr( $index ); ?>][inspiry_floor_plan_size_postfix]" value="<?php echo esc_attr( $floor_plan_size_postfix ); ?>">
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="inspiry-field">
                                <label for="inspiry_floor_plan_bedrooms_<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $floor_bedrooms_label ); ?></label>
                                <input type="text" id="inspiry_floor_plan_bedrooms_<?php echo esc_attr( $index ); ?>" name="inspiry_floor_plans[<?php echo esc_attr( $index ); ?>][inspiry_floor_plan_bedrooms]" value="<?php echo esc_attr( $floor_plan_bedrooms ); ?>">
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="inspiry-field">
                                <label for="inspiry_floor_plan_bathrooms_<?php echo esc_attr( $index ); ?>"><?php echo esc_html( $floor_bathrooms_label ); ?></label>
                                <input type="text" id="inspiry_floor_plan_bathrooms_<?php echo esc_attr( $index ); ?>" name="inspiry_floor_plans[<?php echo esc_attr( $index ); ?>][inspiry_floor_plan_bathrooms]" value="<?php echo esc_attr( $floor_plan_bathrooms ); ?>">
                            </div>
                        </div>
                    </div>
                </div>
...
if ( ! function_exists( 'inspiry_floor_plans' ) ) {
	...
	function inspiry_floor_plans() {
		$floor_plan_section_label = get_option( 'realhomes_submit_property_floor_plans_section_label' );
		if ( empty( $floor_plan_section_label ) ) {
			$floor_plan_section_label = esc_html__( 'Floor Plans', 'framework' );
		}
		?>
        <div class="inspiry-floor-plans-group-wrapper">
            <label class="label-boxed"><?php echo esc_html( $floor_plan_section_label ); ?></label>
...
if ( ! function_exists( 'inspiry_render_property_attachments' ) ) {
	...
	function inspiry_render_property_attachments() {
		...
		$property_attachments_label = get_option( 'realhomes_submit_property_attachments_label' );
		if ( empty( $property_attachments_label ) ) {
			$property_attachments_label = esc_html__( 'Property Attachments', 'framework' );
		}
		?>
        <div class="property-attachments-container-wrap">
            <label><?php echo esc_html( $property_attachments_label ); ?></label>
...
if ( ! function_exists( 'inspiry_additional_details' ) ) {
	...
	function inspiry_additional_details() { 
		$additional_details_label = get_option( 'realhomes_submit_property_additional_details_label' );
		if ( empty( $additional_details_label ) ) {
			$additional_details_label = esc_html__( 'Additional Details', 'framework' );
		}
		?>
        <div class="inspiry-details-wrapper">
            <label><?php echo esc_html( $additional_details_label ); ?></label>
