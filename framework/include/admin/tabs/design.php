...
<div class="inspiry-page-inner-wrap inspiry-design-content-wrap">
    ...
    <?php
    $inspiry_design = get_option( 'inspiry_design_variation', REALHOMES_DESIGN_VARIATION );
    if ( isset( $_POST['inspiry_settings_nonce'] ) && wp_verify_nonce( $_POST['inspiry_settings_nonce'], 'inspiry_settings_action' ) ) {
	    if ( isset( $_POST['rh_design'] ) && ! empty( $_POST['rh_design'] ) ) {
		    $inspiry_design = esc_html( $_POST['rh_design'] );
    ...
    <form id="inspiry-wp-settings-form" method="POST" action="<?php menu_page_url( 'realhomes-design' ); ?>">
        <div class="row">
            ...
            <div class="col-4">
                <div class="design-wrapper">
                    <h3><?php esc_html_e( 'Ultra', 'framework' ); ?></h3>
                    <label class="rh_design__label" for="rh_design_ultra">
                        <input type="radio" name="rh_design" id="rh_design_ultra" class="rh_design__radio hide" value="ultra" <?php checked( $inspiry_design, 'ultra' ) ?>>
                        <img class="rh_design__img" src="<?php echo esc_url( get_template_directory_uri() . '/framework/include/admin/images/design-ultra.jpg' ); ?>">
                    </label>
                </div>
            </div>
        </div>
