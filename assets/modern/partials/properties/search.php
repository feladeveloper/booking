...
<?php
	...
	if ( inspiry_is_search_page_map_visible() ) {
		if( 'list' === $search_results_page_layout ) {
			$rh_page_class = ' listing__list_fullwidth';
		}
	} else {
		$rh_page_class = ' rh_page__listing_page-no-map';
	}
	?>
    <div class="rh_page rh_page__listing_page<?php echo esc_attr( $rh_page_class ); ?>" data-search-url="<?php echo inspiry_get_search_page_url(); ?>">
        ...
        if( 'grid' === $search_results_page_layout  ){
	        $listing_page_class .= ' rh_page__listing_grid rh_page__listing_grid-three-column';
        ...
        <?php realhomes_ajax_pagination( $search_query->max_num_pages, $search_query ); ?>
